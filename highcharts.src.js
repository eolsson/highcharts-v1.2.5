			...
			tooltip: {
				...
				formatter: function () {
					var pThis = this,
						...
						xAxis = series.xAxis,
						x = pThis.x;
					return '<b>' + (pThis.point.name || series.name) + '</b><br/>' + (defined(x) ? 'X value: ' + (xAxis && xAxis.options.type == 'datetime' ? dateFormat('%Y-%m-%d %H:%M:%S', x) : x) + '<br/>' : '') + 'Y value: ' + pThis.y;
				},
...
*/
	...
	function setStyles(el, styles) {
		...
		if (isIE) {
			if (styles.opacity !== undefined) styles.filter = 'alpha(opacity=' + (styles.opacity * 100) + ')';
		}
	...
	var Layer = function (name, appendTo, props, styles) {
		var layer = this,
			...
			styles = extend({
				...
				//appendTo.offsetWidth + PX,
				height: appendToStyle.height,
				//appendTo.offsetHeight + PX,
				position: ABSOLUTE,
				top: 0,
				left: 0,
				margin: 0,
	...
	Layer.prototype = {
		...
		drawLine: function (x1, y1, x2, y2, color, width) {
			...
			ctx.lineWidth = width;
			ctx.lineCap = 'round';
			/* If this is not set, plotBands appear
			...
			if (str || str === 0) {
				...
				if (!rotation || CSStransform) {
					...
					var radians = (rotation || 0) * math.PI * 2 / 360,
						...
						length = layer.width,
						// the text is not likely to be longer than this
	...
	function Chart(options) {
		...
		function getContainer() {
			...
			(renderToOffsetHeight > marginTop + marginBottom ? renderToOffsetHeight : 0) || 400;
		...
		function render() {
			...
			marginLeft, marginTop, plotWidth, plotHeight, null, null, null, optionsChart.plotBackgroundColor, null, plotBackground);

			// Plot area border
			(new Layer('plot-border', container, null, {
				zIndex: 4 // in front of grid lines and graphs, behind axis lines
			})).drawRect(
			marginLeft, marginTop, plotWidth, plotHeight, optionsChart.plotBorderColor, optionsChart.plotBorderWidth, null, null, optionsChart.plotShadow);


			// Printing CSS for IE
			if (isIE) addCSSRule('.highcharts-image-map', {
				display: 'none'
			}, 'print');
			...
			// Credits
			if (credits.enabled && !chart.credits) chart.credits = createElement('a', {
		...
		function Axis(chart, options) {
			...
			function setOptions() {
				...
				horiz ? (opposite ? defaultTopAxisOptions : defaultBottomAxisOptions) : (opposite ? defaultRightAxisOptions : defaultLeftAxisOptions), options);
			};
						...
						if (serie.isCartesian) { // line, column etc. need axes, pie doesn't
							each(serie.data, function (point, i) {
								...
								// y axis
									...
									if (stacking) typeStack[pointX] = typeStack[pointX] ? typeStack[pointX] + pointY : pointY;
			...
			function addTick(pos, tickPos, color, width, len, withLabel, index) {
				...
				if (withLabel && labelOptions.enabled) {
					...
					str, x1 + labelOptions.x - (tickmarkOffset && horiz ? tickmarkOffset * transA * (reversed ? -1 : 1) : 0), y1 + labelOptions.y - (tickmarkOffset && !horiz ? tickmarkOffset * transA * (reversed ? 1 : -1) : 0), labelOptions.style, labelOptions.rotation, labelOptions.align);
				}
			...
			function setLinearTickPositions() {
				...
				var correctFloat = function (num) { // JS round off float errors
					...
					i, roundedMin = mathFloor(min / tickInterval) * tickInterval,
			...
			function setScale() {
				...
				if (!categories && !usePercentage) {
					length = (max - min) || 1;
					if (!defined(options.min) && minPadding && (dataMin < 0 || !ignoreMinPadding)) min -= length * minPadding;
					if (!defined(options.max) && maxPadding && (dataMax > 0 || !ignoreMaxPadding)) max += length * maxPadding;
				}

				// tickInterval
				if (categories || min == max) tickInterval = 1;
				else tickInterval = options.tickInterval == 'auto' ? (max - min) * options.tickPixelInterval / axisLength : options.tickInterval;
		...
		function reset() {
			...
			function render() {
				...
				if (hasData) {
					...
					each(tickPositions, function (pos, index) {
						...
						// add the tick mark
						addTick(
						pos, options.tickPosition, options.tickColor, options.tickWidth, options.tickLength, !((pos == min && !options.showFirstLabel) || (pos == max && !options.showLastLabel)), index);
					});
				...
				if (lineWidth) {
					...
					axisLayer.drawLine(
					horiz ? marginLeft : lineLeft, horiz ? lineTop : marginTop, horiz ? chartWidth - marginRight : lineLeft, horiz ? lineTop : chartHeight - marginBottom, options.lineColor, lineWidth);
				}
				...
				if (axisTitle && axisTitle.enabled && axisTitle.text) {
					...
					var offAxis = (horiz ? marginTop + plotHeight : marginLeft) + (horiz ? 1 : -1) * // horizontal axis reverses the margin
					(opposite ? -1 : 1) * // so does opposite axes
					axisTitle.margin - (isIE ? parseInt(
					axisTitle.style.fontSize || axisTitle.style.font.replace(/^[a-z ]+/, '')) / 3 : 0); // preliminary fix for vml's centerline
					axisLayer.addText(
					axisTitle.text, horiz ? alongAxis : offAxis + (opposite ? plotWidth : 0) + offset, // x
					horiz ? offAxis - (opposite ? plotHeight : 0) + offset : alongAxis, // y
					axisTitle.style, axisTitle.rotation || 0, {
						low: 'left',
						middle: 'center',
						high: 'right'
					}[axisTitle.align]);
			...
			var axis = this,
				...
				transB = horiz ? marginLeft : marginBottom,
				// translation addend
				...
				ignoreMaxPadding, usePercentage, events = options.events,
				eventType, plotBands = options.plotBands || [],
				plotLines = options.plotLines || [],
				tickInterval, minorTickInterval, magnitude, tickPositions, // array containing predefined positions
				tickAmount, zoom = 1,
		...
		function MouseTracker(chart, options) {
			...
			function normalizeMouseEvent(e) {
				...
				// pageX and pageY
				if (!e.pageX) e.pageX = e.clientX + (doc.documentElement.scrollLeft || doc.body.scrollLeft);

				if (!e.pageY) e.pageY = e.clientY + (doc.documentElement.scrollTop || doc.body.scrollTop);
			...
			function getMouseCoordinates(e) {
				...
				each(axes, function (axis, i) {
					...
					coordinates[isXAxis ? 'xAxis' : 'yAxis'].push({
						axis: axis,
						value: translate(
						isHorizontal ? e.pageX - position.x - marginLeft : plotHeight - e.pageY + position.y + marginTop, true)
					});
				});
				return coordinates;
			}
		...
		*/
				...
				*/
				...
				imagemap.onmousemove = function (e) {
					...
					if (mouseIsDown) { // make selection
						...
						Math.pow(mouseDownX - e.pageX, 2) + Math.pow(mouseDownY - e.pageY, 2)) > 10;
						...
						if (hasDragged) {
							...
							each(axes, function (axis, i) {
								var translate = axis.translate,
									...
									isHorizontal ? selectionLeft : plotHeight - selectionTop - selectionHeight, true),
									selectionMax = translate(
									isHorizontal ? selectionLeft + selectionWidth : plotHeight - selectionTop, true);
			...
			function onmousemove(e) {
				...
				if (series) {
					...
					inverted ? e.pageY - position.y - marginTop : e.pageX - position.x - marginLeft];
			...
			// Run MouseTracker
			var activePoint, mouseDownX, mouseDownY, hasDragged, selectionMarker,
			/*dragPoint,
		...
		var Legend = function (chart) {
				...
				function drawGraphics(clear) {
					...
					if (series.length) {
						...
						if (options.borderWidth || options.backgroundColor) legendLayer.drawRect(
						dom.offsetLeft, dom.offsetTop, dom.offsetWidth, dom.offsetHeight, options.borderColor, options.borderWidth, options.borderRadius, options.backgroundColor, options.shadow);
						...
						each(allItems, function (item) {
							...
							// draw the line
							if (!item.simpleSymbol && item.options && item.options.lineWidth) legendLayer.drawLine(
							symbolX, symbolY, symbolX + symbolWidth, symbolY, color, item.options.lineWidth);
							// draw a simple symbol
							if (item.simpleSymbol) // bar|pie|area|column
							legendLayer.drawRect(
							symbolX, symbolY - 6, 16, 12, null, 0, 2, color);

							// draw the marker
							else if (item.options && item.options.marker && item.options.marker.enabled) item.drawMarker(
							legendLayer, symbolX + symbolWidth / 2, symbolY, merge(item.options.marker, isHidden ? {
								fillColor: color,
								lineColor: color
							} : null));
						});
			...
			function refresh(point, series) {
				...
				if (text === false || !show) {
					...
					if (boxWidth > (boxLayer.w || 0) + 20 || boxWidth < (boxLayer.w || 0) - 20 || boxHeight > boxLayer.h || boxLayer.c != borderColor || oldInnerDivHeight != innerDiv.offsetHeight) {
						boxLayer.clear();
						boxLayer.drawRect(
						borderWidth / 2, borderWidth / 2, boxWidth + 20, boxHeight, borderColor, borderWidth, options.borderRadius, options.backgroundColor, options.shadow);
		...
		var chart = this,
			...
			//chartHeight,
			//chartWidth,
			loadingLayer, plotLayer, plotHeight, plotWidth,
			//ctx,
			tracker, trackerImage, legend,
			//xAxis,
			//yAxis,
			position = getPosition(container),
	...
	Series.prototype = {
		...
		setOptions: function (options) {
			...
			if (hoverSeriesMarkerOptions.lineWidth === undefined) hoverSeriesMarkerOptions.lineWidth = normalSeriesMarkerOptions.lineWidth + 1;
			if (hoverSeriesMarkerOptions.radius === undefined) hoverSeriesMarkerOptions.radius = normalSeriesMarkerOptions.radius + 1;
		...
		getSymbol: function () {
			...
			this.symbol = symbol;
			if (symbolCounter >= defaultSymbols.length) symbolCounter = 0;
		},
		...
		translate: function () {
			...
			each(this.data, function (point) {
				...
				point.clientX = chart.inverted ? chart.plotHeight - point.plotX + chart.plotTop : point.plotX + chart.plotLeft; // for mouse tracking
				// some API data
		...
		setTooltipPoints: function (renew) {
			var series = this,
				...
				low, high, tooltipPoints = []; // a lookup array for each pixel in the x dimension
			// renew
			...
			each(data, function (point, i) {
				...
				mathFloor((point.plotX + (data[i + 1] ? data[i + 1].plotX : plotSize)) / 2)) : plotSize;
		...
		drawLine: function (state) {
			...
			// get state options
			if (state) options = merge(options, options.states[state]);
			...
			each(series.segments, function (segment) {
				...
				each(segment, function (point, i) {
					if (i && options.step) {
						...
						line.push(
						inverted ? chart.plotWidth - lastPoint.plotY : point.plotX, inverted ? chart.plotHeight - point.plotX : lastPoint.plotY);
					}

					line.push(
					inverted ? chart.plotWidth - point.plotY : point.plotX, inverted ? chart.plotHeight - point.plotX : point.plotY);
				});
				...
				if (/area/.test(series.type)) {
					...
					if (options.stacking && series.type != 'areaspline') {
						...
						area.push(
						inverted ? y0 : segment[segment.length - 1].plotX, inverted ? chart.plotHeight - segment[segment.length - 1].plotX : y0, inverted ? y0 : segment[0].plotX, inverted ? chart.plotHeight - segment[0].plotX : y0);
					}
	...
	return state;
		...
		drawMarker: function (layer, x, y, options) {
			...
			layer.drawSymbol(
			options.symbol, x, y, options.radius, options.lineWidth, options.lineColor, options.fillColor);
		},
		...
		drawDataLabels: function () {
			if (this.options.dataLabels.enabled) {
				...
				each(data, function (point) {
					...
					str, x, y, options.style, options.rotation, align);
		...
		drawPointState: function (point, state, layer) {
			...
			if (state) {
				...
				if (isHoverState && pointStateOptions.radius === undefined) pointStateOptions.radius = seriesStateOptions.radius + 2;

				// merge all options
				stateOptions = merge(
				options.marker, point.marker, seriesStateOptions, pointStateOptions);

				// draw the marker
				if (stateOptions && stateOptions.enabled) this.drawMarker(
				layer, inverted ? chart.plotWidth - point.plotY : point.plotX, inverted ? chart.plotHeight - point.plotX : point.plotY, stateOptions);
			}
		},
		...
		redraw: function () {
			...
			/*if (series.chart.options.tooltip.enabled) */
			series.createArea();
			series.clear();
		...
		getAreaCoords: function () {
			...
			each(series.splinedata || series.segments, function (data, i) {
				...
				each([outlineTop, outlineBottom], function (outline) {
					...
					// push the last point
					if (peaks[peaks.length - 1] != data[data.length - 1]) peaks.push(data[data.length - 1]);
					...
					for (i = 0; i < peaks.length; i++) {
						...
						if (i > 0) {
							...
							if (outline[outline.length - 1] && outline[outline.length - 1][0] > x1) {
								...
								if (!isNaN(x1)) outline.push([x1, y1]);
							}
							if (outline[outline.length - 1] && outline[outline.length - 1][0] < x2) outline.push([x2, y2]);
						}
					}
				...
				for (i = 0; i < outlineTop.length; i++) { // top of the area
					coords.push(
					inverted ? plotWidth - outlineTop[i][1] : outlineTop[i][0], inverted ? plotHeight - outlineTop[i][0] : outlineTop[i][1]);

				}
				for (i = outlineBottom.length - 1; i >= 0; i--) { // bottom of the area
					coords.push(
					inverted ? plotWidth - outlineBottom[i][1] : outlineBottom[i][0], inverted ? plotHeight - outlineBottom[i][0] : outlineBottom[i][1]);
				}
	...
	var SplineSeries = extendClass(Series, {
		...
		getSplineData: function () {
			...
			each(series.segments, function (data) {
				...
				num ? (new SplineHelper(croppedData)).get(num) : [] : data);
			});
	...
	SplineHelper.prototype = {
		...
		interpolate: function (xpoint) {
			...
			while (max - min > 1) {
				var k = (max + min) / 2;
				if (this.xdata[mathFloor(k)] > xpoint) max = k;
				else min = k;
			}
		...
		error: ('Invalid input data for spline. Two or more consecutive input X-values are equal. Each input X-value must differ since from a mathematical point of view it must be a one-to-one mapping, i.e. each X-value must correspond to exactly one Y-value.');
			...
			return a * this.ydata[intMin] + b * this.ydata[intMax] + ((a * a * a - a) * this.y2[intMin] + (b * b * b - b) * this.y2[intMax]) * (h * h) / 6.0;
		}
	...
	var ColumnSeries = extendClass(Series, {
		...
		translate: function () {
			...
			var options = series.options,
				...
				data[1] ? data[closestPoints].plotX - data[closestPoints - 1].plotX : (inverted ? plotHeight : plotWidth)),
				...
				columnIndex = (chart.options.xAxis.reversed ? columnCount - series.columnIndex : series.columnIndex) || 0,
		...
		singlePointLayer.clear();*/
			...
			// a specific layer for the currently active point
			if (state == 'hover') {
				if (!layer) layer = series.singlePointLayer = new Layer('single-point', series.layerGroup.div);
				layer.clear();
			}
			...
			if (state && this.options.states[state]) {
				var options = merge(
				seriesOptions, seriesOptions.states[state], pointOptions);
				layer.drawRect(
				inverted ? chart.plotWidth - point.plotY - point.h : point.plotX, inverted ? chart.plotHeight - point.plotX - point.w : point.plotY, inverted ? point.h : point.w, inverted ? point.w : point.h, options.borderColor, options.borderWidth, options.borderRadius, Color(options.color || this.color).brighten(options.brightness).get(), options.shadow);
			}
		},
	...
	var PieSeries = extendClass(Series, {
		...
		translate: function () {
			...
			positions = map(positions, function (length, i) {
				...
				chart['plot' + (i ? 'Height' : 'Width')] * parseInt(length) / 100 : length;
			});
		...
		drawPointState: function (point, state, layer) {
			...
			stateLayer.clear();*/
				...
				if (state == 'hover') {
					if (!layer) layer = point.stateLayer = new Layer('single-point', point.layer.div);
					layer.clear();
				}
				...
				if (state && series.options.states[state]) {
					...
					this.drawPoint(
					point, layer.getCtx(), options.color || point.color, options.brightness);
				}
